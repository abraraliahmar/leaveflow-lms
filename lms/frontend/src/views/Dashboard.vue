  ...
  <div class="max-w-6xl mx-auto px-4 py-8">
    ...
    <div class="mb-8">
      ...
      <p class="text-gray-500 mt-1 text-sm">
        {{ auth.isEmployer ? 'Manage your team\'s leave requests from here.' : 'Track and manage your leave applications.' }}
      </p>
    </div>
    ...
    <div class="flex gap-3 mb-8">
      <RouterLink v-if="auth.isEmployee" to="/apply" class="btn-primary gap-2">
        ＋ Apply for Leave
      </RouterLink>
      <RouterLink to="/leaves" class="btn-secondary gap-2">
        View {{ auth.isEmployer ? 'All Requests' : 'My Leaves' }}
      </RouterLink>
    </div>
    ...
    <div class="card">
      <div class="px-6 py-4 border-b border-gray-100 flex items-center justify-between">
        <h2 class="font-semibold text-gray-900 font-display">Recent Requests</h2>
      ...
      <div v-else-if="recentLeaves.length === 0" class="py-12 text-center text-gray-400 text-sm">
        No leave applications yet.
      </div>
      <div v-else class="divide-y divide-gray-50">
        <div v-for="leave in recentLeaves" :key="leave._id" class="px-6 py-4 flex items-center justify-between">
          <div class="flex items-center gap-4">
            <div class="w-9 h-9 rounded-full bg-primary-100 flex items-center justify-center text-sm">
